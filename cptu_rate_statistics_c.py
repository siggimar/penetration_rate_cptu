...
def calc_stats( cpts, parameter, x_rng=[None,None,None], unit='mm/s', x_label='Penetration rate' ):    
    ...
    axs.hist( combined_rates, bins=np.arange(bin_range[0], bin_range[1] + bin_width, bin_width), ec='k', fc=(0/255,142/255,194/255), label='Test data - ' + str(round(k-5,-1)) + ' tests' ) #, density=True )
    ...
    print(len(combined_rates))
    print(len(combined_rates[( combined_rates>=5 ) & ( combined_rates<=40 ) ]))
    a=1
    ...
    axs.fill_between( x, 0, 1, where= np.logical_and(x>perc[1][0],x<perc[0][0]), color=perc_colors[1], alpha=0.3, transform=axs.get_xaxis_transform(),zorder=-1)
    axs.fill_between( x, 0, 1, where= np.logical_and(x>perc[0][1],x<perc[1][1]), color=perc_colors[1], alpha=0.3, transform=axs.get_xaxis_transform(),zorder=-1)
    axs.fill_between( x, 0, 1, where= np.logical_and(x>perc[0][0],x<perc[0][1]), color=perc_colors[0], alpha=0.3, transform=axs.get_xaxis_transform(),zorder=-1)

    axs.set_ylim( n_min, n_max ) # 0.25 for pdf variant
    ...
    axs.set_ylabel('Number of ' + x_label.lower() + ' registrations (-)', fontsize=axis_label_size)

    # annotate
    if parameter=='b':
        axs.annotate('Push breaks', xy=(0.2,7000), xycoords='data',
        xytext=(0,75000), textcoords='data', va='top', ha='left', fontsize=annotation_labels_size,
        arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))

        axs.annotate('CPTUs at total-\nsounding rates', xy=(47,7000), xycoords='data',
        xytext=(36,105000), textcoords='data', va='top', ha='left', fontsize=annotation_labels_size,
        arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))

    axs.annotate(str(perc_vals[1][0]) + 'th: ' + str(round(perc[1][0],1)) + unit, xy=(perc[1][0],320000), xycoords='data',
    xytext=(2,330000), textcoords='data', va='top', ha='left', fontsize=annotation_labels_size,
    arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))

    axs.annotate(str(perc_vals[0][0]) + 'th: ' + str(round(perc[0][0],1)) + unit, xy=(perc[0][0],420000), xycoords='data',
    xytext=(2,430000), textcoords='data', va='top', ha='left', fontsize=annotation_labels_size,
    arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))

    axs.annotate('50th: ' + str(round(np.percentile( combined_rates, 50),1)) + unit, xy=(np.percentile( combined_rates, 50),530000), xycoords='data',
    xytext=(2,540000), textcoords='data', va='top', ha='left', fontsize=annotation_labels_size,
    arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))

    axs.annotate(str(perc_vals[0][1]) + 'th: ' + str(round(perc[0][1],1)) + unit, xy=(perc[0][1],420000-n_inc), xycoords='data',
    xytext=(round(perc[1][1],1)+3*bin_width,430000-n_inc), textcoords='data', va='top', ha='left', fontsize=annotation_labels_size,
    arrowprops=dict(arrowstyle="->", connectionstyle="arc3"))

    axs.annotate(str(perc_vals[1][1]) + 'th: ' + str(round(perc[1][1],1)) + unit, xy=(perc[1][1],320000-n_inc), xycoords='data',
    ...
    axs.xaxis.set_ticks( np.arange( x_rng[0], x_rng[1]+1, x_rng[2] ) )

    #axs[0].legend(loc='lower right')
    axs.legend(loc='upper right')

    #axs[0].text( 2, 2.5, 'A)', size=16, bbox=dict(boxstyle="square",ec=(1, 1, 1,.7),fc=(1, 1, 1,.7)))
    #axs.text(-2, 570000, 'B)', size=16, bbox=dict(boxstyle="square",ec=(1, 1, 1),fc=(1, 1, 1)))
    
    
    fig.savefig('statistics_1.png', dpi=600)
